...
//    var vcc = FirstView()
    ...
    override func viewDidLoad() {
        ...
        let image_app1 = UIImage(named: "App1.png") as UIImage!
        ...
        let image_back = UIImage(named: "back.png") as UIImage!
        ...
        let titlea1 = UILabel(frame: CGRect(x: centerP.x / 10, y: 165, width: 190, height: 90))
        titlea1.text = "Snake"
        titlea1.center = CGPoint(x: centerP.x / 3.5, y: 100)
        titlea1.textAlignment = NSTextAlignment.Center
        let titlea1a = UILabel(frame: CGRect(x: centerP.x / 10, y: 165, width: 190, height: 90))
        titlea1a.text = "2025"
        titlea1a.center = CGPoint(x: centerP.x / 3.5, y: 115)
        titlea1a.textAlignment = NSTextAlignment.Center
        // Do any additional setup after loading the view.
        let scrollView = UIScrollView(frame: viewRect)
        scrollView.delegate = self
        scrollView.pagingEnabled = true
        scrollView.contentSize = CGSizeMake(self.viewWidth, self.viewHeight)
        self.view.addSubview(scrollView)
        self.scrollView = scrollView
        let subScrollView = UIScrollView(frame: CGRect(x: 0, y: 50, width: centerP.x * 2, height: centerP.y * 2 - 20))
